...
class FitnessFunction:
    ...
    def calculate_fitness_with_constraints(
        ...
        for point in getattr(route, "delivery_points", []):
            ...
            try:
                ...
                FitnessFunction.logger.warning("Produto com peso inválido; considerando 0g.")
        ...
        total_cost = (total_distance + 1e-6) + weight_penalty + volume_penalty + final_priority_penalty
        ...
        FitnessFunction.logger.debug(
            ...
            total_distance, total_weight_g, MAX_W, total_volume_cm3, MAX_V,
            weight_overshoot, volume_overshoot, weight_penalty, volume_penalty, final_priority_penalty, total_cost
        )
    ...
    def _roundtrip_distance(delivery_points: List, depot: DeliveryPoint) -> float:
        ...
        for i in range(len(delivery_points) - 1):
            segment_distance = delivery_points[i].distancia_euclidean(delivery_points[i + 1])
    ...
    def _split_with_vehicle_choice(order: List, deposito, fleet: List[VehicleType]) -> Tuple[float, List[Route], Dict[str, int], float]:
        ...
        dp_cost, prev_idx, vehicle_at = FitnessFunction._compute_dp(order, deposito, fleet)
        ...
        routes, usage, split_sequences = FitnessFunction._reconstruct_routes(order, prev_idx, vehicle_at)

        # 3) Penalidades (excesso de frota por tipo e prioridade tardia)
        penalties_total, weighted_priority_penalty = FitnessFunction._compute_penalties(usage, split_sequences, fleet)
    ...
    # ------------------------- Helpers privados -------------------------
    @staticmethod
    def _compute_dp(order: List, depot: DeliveryPoint, fleet: List[VehicleType]) -> Tuple[List[float], List[int], List[Optional[VehicleType]]]:
        """
        ...
        for i in range(n):
            ...
            for j in range(i + 1, n + 1):
                ...
                best_vehicle = FitnessFunction._find_cheapest_vehicle(segment_distance, fleet)
                if best_vehicle is None:
                    continue
                
                # Atualizar solução se encontrou custo menor
                candidate_cost = dp_cost[i] + (best_vehicle.cost_per_km * segment_distance)
    ...
    @staticmethod
    def _initialize_dp_arrays(n: int) -> Tuple[List[float], List[int], List[Optional[VehicleType]]]:
    ...
    @staticmethod
    def _setup_dp_optimization(fleet: List[VehicleType]) -> Tuple[Dict[Tuple[int, int], float], float]:
    ...
    def _get_cached_roundtrip_distance(
        start_idx: int, end_idx: int, order: List, depot: DeliveryPoint, 
    ...
    @staticmethod
    def _find_cheapest_vehicle(distance: float, fleet: List[VehicleType]) -> Optional[VehicleType]:
    ...
    @staticmethod
    def _reconstruct_routes(order: List, prev_idx: List[int], vehicle_at: List[Optional[VehicleType]]) -> Tuple[List[Route], Dict[str, int], List[List[DeliveryPoint]]]:
        """
        ...
        while current_pos > 0:
            ...
            vehicle_usage[assigned_vehicle.name] = vehicle_usage.get(assigned_vehicle.name, 0) + 1
    ...
    @staticmethod
    def _compute_penalties(vehicle_usage: Dict[str, int], split_sequences: List[List[DeliveryPoint]], fleet: List[VehicleType]) -> Tuple[float, float]:
        """
        ...
        fleet_penalty = FitnessFunction._calculate_fleet_excess_penalty(vehicle_usage, fleet)
    ...
    @staticmethod
    def _calculate_fleet_excess_penalty(usage: Dict[str, int], fleet: List[VehicleType]) -> float:
        """
    ...
    @staticmethod
    def _calculate_priority_penalty(split_sequences: List[List[DeliveryPoint]]) -> float:
        """
    ...
    def calculate_fitness_with_fleet(route: Route, deposito, fleet: List[VehicleType]) -> Tuple[float, List[Route], Dict[str, int]]:
        ...
        FitnessFunction._log_solution_details(total_cost, optimized_routes, vehicle_usage, priority_penalty)
    ...
    @staticmethod
    def _log_solution_details(total_cost: float, routes: List[Route], usage: Dict[str, int], penalty: float) -> None:
